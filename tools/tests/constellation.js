...
scenario('ciel — trois états : acquise, ouverte, devinée', () => {
  ...
  eq('et le SVG n’écrit que les noms qu’il a le droit d’écrire, et aucun au moyeu',
     tous('etoile-nom').length, jeu.CIEL.length - devinees.length - 1);
...
scenario('constellation — le ciel se dessine, et il est plus grand que l’écran', () => {
  ...
  eq('il n’y a qu’un nom et un prix par étoile lisible, sauf au moyeu',
     tous('etoile-nom').length, jeu.CIEL.length - devinees.length - 1);
